...
| ☐ | `@trace:` | `requirements/mission.deal` | Dependency (M2 · designed) |
...
| ✓ | Item | Where |
|---|---|---|
| ☐ | 3 Python sims on deal-sim SDK | `simulations/{power,thermal,comms}/` |
| ☐ | 3 MATLAB sims via `matlab -batch` adapter | `simulations/{adcs,orbit}/` |
| ☐ | graceful-skip path exercised once (exit 3 → skip.json, D-72) | CI note in README |
| ☐ | v0 contract: alphabetical keys (D-18), staleness keys (D-83), repro tiers (D-75) | all sims |
| ☐ | `deal.sims.toml`: binds_to, inputs/outputs, auto_run | `simulations/deal.sims.toml` |
| ☐ | evidence baseline captured (`deal evidence baseline initial`) | Phase 6 |
